...
void __fastcall TForm3::FormCreate(TObject *Sender)
{
	...
	char Path[] = "base\\Заказчики и доставщики.mdb";
	file=fopen(Path, "r");
	ConStr = ConStr + Path;
	ADOConnection1->ConnectionString=ConStr;
	ADOConnection1->Connected=true;
	ADOTable1->TableName="Заказчики";
	...
	ADOTable2->TableName="Доставщики";
...
void __fastcall TForm3::Button2Click(TObject *Sender)
{
	...
	while (!ADOTable1->Eof)
	{
		Form4->ComboBox1->Items->Append(Form3->ADOTable1->Fields->FieldByName("ID заказа")->AsString);
		ADOTable1->Next();
	}
	Form4->ShowModal();
}
...
{
	if (MessageDlg("Удалить строку БД?",mtConfirmation,TMsgDlgButtons()<<mbYes<<mbNo,0)==mrYes)
	ADOTable1->Delete();
}
//---------------------------------------------------------------------------
